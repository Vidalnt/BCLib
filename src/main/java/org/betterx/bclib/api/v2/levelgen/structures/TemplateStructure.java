...
public abstract class TemplateStructure extends Structure {
    ...
    public static boolean isValidBiome(
    ...
    public static <T extends TemplateStructure> Codec<T> simpleTemplateCodec(
        BiFunction<StructureSettings, List<Config>, T> instancer
    ) {
            ...
            instance
                .group(
                    ...
                    ExtraCodecs.nonEmptyList(Config.CODEC.listOf())
                        .fieldOf("configs")
                        .forGetter((T ruinedPortalStructure) ->
                            ruinedPortalStructure.configs
                        )
                )
    ...
    protected TemplateStructure(
        ...
        this(
            structureSettings,
            List.of(new Config(location, offsetY, type, chance))
        );
    }
    ...
    protected boolean isLavaPlaceable(BlockState state, BlockState before) {
        return (
            (state == null || state.is(Blocks.AIR)) && before.is(Blocks.LAVA)
        );
    }
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext ctx) {
        ...
        if (!hasValidBiomeAtRandomHeight(ctx, x, z)) return Optional.empty();
        ...
        StructureTemplate structureTemplate = ctx
            .structureTemplateManager()
            .getOrCreate(config.location);
        ...
        final int seaLevel =
            ...
            (searchStep > 0
                ? 0
                : (structureTemplate.getSize(Rotation.NONE).getY() +
                      config.offsetY));
        final int maxHeight =
        ...
        BoundingBox boundingBox = structureTemplate.getBoundingBox(
            centerPos,
            rotation,
            halfSize,
            mirror
        ...
        var noiseColumns = ImmutableList.of(
            new BlockPos(
        ...
        int y = noiseColumns
            .stream()
            .map(column ->
                findY(column, isCorrectBase, searchStep, seaLevel, maxHeight)
            )
            .reduce(
        ...
        int baseCount = noiseColumns
            .stream()
            .map(column ->
                isCorrectBase.test(null, column.getBlock(y - searchStep))
            )
            .filter(b -> b)
            .map(b -> 1)
            .reduce(0, (p, c) -> p + c);
        ...
        float airRatio =
            noiseColumns
                .stream()
                .map(column ->
                    airRatio(column, y, boundingBox.getYSpan(), searchStep)
                )
                .reduce(0.0f, (p, c) -> p + c) /
            noiseColumns.size();
        ...
        return Optional.of(
            new GenerationStub(centerPos, structurePiecesBuilder ->
                structurePiecesBuilder.addPiece(
                    new TemplatePiece(
                        ctx.structureTemplateManager(),
                        config.location,
                        centerPos.offset(0, config.offsetY, 0),
                        rotation,
                        mirror,
                        halfSize,
                        erosion,
    ...
    private boolean hasValidBiomeAtRandomHeight(
        GenerationContext ctx,
        int x,
        int z
    ) {
        final int randomY = ctx
            .random()
