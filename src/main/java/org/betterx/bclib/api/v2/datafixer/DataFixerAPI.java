...
public class DataFixerAPI {
    ...
    private static boolean wrapCall(
        ...
        try {
            ...
            BCLib.LOGGER.warn(
                "Failed to unlock access to level {}",
                levelID,
                e
            );
        }
    ...
    public static boolean fixData(
        ...
        return wrapCall(levelSource, levelID, levelStorageAccess ->
            fixData(levelStorageAccess, showUI, onResume)
        );
    }
    ...
    public static boolean fixData(
        ...
        File levelPath = levelStorageAccess
    ...
    private static void makeBackupAndShowToast(
        ...
        try (
            LevelStorageSource.LevelStorageAccess access =
                storageSource.createAccess(levelID);
        ) {
            didOpen = true;
                ...
                if (showUI) {
                    ...
                    progress = new AtomicProgressListener() {
                        ...
                        public void incAtomic(int maxProgress) {
                            int percentage =
                                (100 * counter.incrementAndGet()) / maxProgress;
                ...
                if (createBackup) {
                    progress.progressStage(
                        Component.translatable(
                            "message.bclib.datafixer.progress.waitbackup"
                        )
                    );
    ...
    @Environment(EnvType.CLIENT)
    private static void showLevelFixErrorScreen(
        State state,
        Listener onContinue
    ) {
        Minecraft.getInstance().setScreen(
            new LevelFixErrorScreen(
                Minecraft.getInstance().screen,
    ...
    private static MigrationProfile getMigrationProfile() {
        final CompoundTag patchConfig = WorldConfig.getCompoundTag(
            BCLib.C,
    ...
    @Environment(EnvType.CLIENT)
    static void showBackupWarning(
        String levelID,
        BiConsumer<Boolean, Boolean> whenFinished
    ) {
        Minecraft.getInstance().setScreen(
            new ConfirmFixScreen(null, whenFinished::accept)
        );
    }
    ...
    private static State runDataFixes(
        ...
        progress.progressStage(
            Component.translatable("message.bclib.datafixer.progress.reading")
        );
        ...
        progress.progressStage(
            Component.translatable("message.bclib.datafixer.progress.players")
        );
        RegionStorageInfo regionStorageInfo = new RegionStorageInfo(
            levelID,
            ResourceKey.create(
                Registries.DIMENSION,
                BCLib.makeID("world_fixer")
            ),
            "mca"
        );
        players
        ...
        progress.progressStage(
            Component.translatable("message.bclib.datafixer.progress.level")
        );
        fixLevel(profile, state, dir, regionStorageInfo);
        progress.incAtomic(maxProgress);

        progress.progressStage(
            Component.translatable("message.bclib.datafixer.progress.worlddata")
        );
        try {
            ...
            state.didFail = true;
            state.addError(
                "Failed fixing worldconfig (" + e.getMessage() + ")"
            );
        ...
        progress.progressStage(
            Component.translatable("message.bclib.datafixer.progress.regions")
        );
        regions
            .parallelStream()
            .forEach(file -> {
        ...
        if (!state.didFail) {
            progress.progressStage(
                Component.translatable(
                    "message.bclib.datafixer.progress.saving"
                )
            );
            profile.markApplied();
            WorldConfig.saveFile(BCLib.C);
    ...
    private static void fixLevel(
        ...
        try {
            ...
            state.addError(
                "Failed fixing Level-Data in level.dat (" + e.getMessage() + ")"
            );
            state.didFail = true;
            e.printStackTrace();
        }
    ...
    private static void fixPlayerNbt(
        CompoundTag player,
        boolean[] changed,
        MigrationProfile data
    ) {
        ...
        if (player.contains("recipeBook")) {
            player
                .getCompound("recipeBook")
                .ifPresent(recipeBook -> {
                    changed[0] |= fixStringIDList(recipeBook, "recipes", data);
                    changed[0] |= fixStringIDList(
                        recipeBook,
                        "toBeDisplayed",
                        data
                    );
    ...
    static boolean fixStringIDList(
        CompoundTag root,
        String name,
        MigrationProfile data
    ) {
            ...
            for (int x = 0; x < 32; x++) {
                for (int z = 0; z < 32; z++) {
                    ...
                    if (region.hasChunk(pos) && !state.didFail) {
                        DataInputStream input = region.getChunkDataInputStream(
                            pos
                        );
                        ...
                        sections.forEach(tag -> {
                            ListTag palette = ((CompoundTag) tag).getList(
                                "Palette"
                            ).orElse(new ListTag());
                            palette.forEach(blockTag -> {
                                CompoundTag blockTagCompound =
                                    ((CompoundTag) blockTag);
                                changed[0] |= data.replaceStringFromIDs(
                                    blockTagCompound,
                                    "Name"
                                );
                            });
                            ...
                            ((CompoundTag) tag).getList(
                                ...
                                try {
                                    ...
                                    BCLib.LOGGER.error(
                                        "Failed fixing BlockState in " + pos
                                    );
                                    state.addError(
                                        "Failed fixing BlockState in " +
                                            pos +
                                            " (" +
                                            e.getMessage() +
                                            ")"
                        ...
                        if (changed[0]) {
                            ...
                            DataOutputStream output =
                                region.getChunkDataOutputStream(pos);
    ...
    static CompoundTag getPatchData() {
        if (patchConfTag == null) {
            patchConfTag = WorldConfig.getCompoundTag(
                BCLib.C,
                Configs.MAIN_PATCH_CATEGORY
            );
        }
    ...
    static void fixItemArrayWithID(
        ListTag items,
        boolean[] changed,
        MigrationProfile data,
        boolean recursive
    ...
    static void fixID(
        ...
        if (recursive) {
            tag
                .getList("Items")
                .ifPresent(items ->
                    fixItemArrayWithID(items, changed, data, true)
                );
        }
        if (recursive) {
            tag
        ...
        if (tag.contains("tag")) {
            ...
            if (entityTag.contains("BlockEntityTag")) {
                CompoundTag blockEntityTag = (CompoundTag) entityTag.get(
                    "BlockEntityTag"
                );
