...
public class StructureNBT {
    ...
    private static final Map<Identifier, StructureNBT> STRUCTURE_CACHE =
        Maps.newHashMap();

    public static StructureNBT create(Identifier location) {
        return STRUCTURE_CACHE.computeIfAbsent(location, r ->
            new StructureNBT(r)
        );
    }

    public boolean generateCentered(
    ...
    private BlockPos getCenteredPos(
        ...
        MutableBlockPos blockpos2 = new MutableBlockPos().set(
            structure.getSize()
        );
        if (mirror == Mirror.FRONT_BACK) blockpos2.setX(-blockpos2.getX());
        if (mirror == Mirror.LEFT_RIGHT) blockpos2.setZ(-blockpos2.getZ());
        blockpos2.set(blockpos2.rotate(rotation));
        return pos.offset(-blockpos2.getX() >> 1, 0, -blockpos2.getZ() >> 1);
    }

    private static final Map<Identifier, StructureTemplate> READER_CACHE =
        Maps.newHashMap();

    private static StructureTemplate readStructureFromJar(Identifier resource) {
        return READER_CACHE.computeIfAbsent(resource, r ->
            _readStructureFromJar(r)
        );
    }
    ...
    private static StructureTemplate _readStructureFromJar(
        Identifier resource
    ) {
        try {
            InputStream inputstream = MinecraftServer.class.getResourceAsStream(
                "/" + getStructurePath(resource) + ".nbt"
            );
     ...
     */
    public static List<StructureNBT> createResourcesFrom(
        Identifier resource,
        int recursionDepth
    ) {
        ...
        final URL url = MinecraftServer.class.getClassLoader().getResource(
            resourceFolder
        );
        if (url != null) {
            final URI uri;
            ...
            if (Files.isDirectory(myPath)) {
                try {
                    ...
                    return Files.walk(
                        myPath,
                        recursionDepth <= 0 ? Integer.MAX_VALUE : recursionDepth
                    )
                        .filter(p -> Files.isRegularFile(p))
                        .map(p -> {
                            if (p.isAbsolute()) return Path.of(uri)
                                .relativize(p)
                                .toString();
                            else {
                                return p
                                    .toString()
                                    .replace(resourceFolder, "")
                                    .replaceAll("^/+", "");
                            }
                        })
    ...
    public BoundingBox getCenteredBoundingBox(
        BlockPos pos,
        Rotation rotation,
        Mirror mirror
    ) {
