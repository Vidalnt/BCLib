...
public class StructureWorldNBT extends StructureNBT {
        ...
        RecordCodecBuilder.create(instance ->
            instance
                .group(
                    ...
                    StructurePlacementType.CODEC.fieldOf("placement")
    ...
    private static final Map<String, StructureWorldNBT> READER_CACHE =
        Maps.newHashMap();
    ...
    public static StructureWorldNBT create(
        ...
        return READER_CACHE.computeIfAbsent(key, r ->
            new StructureWorldNBT(location, offsetY, type, chance)
        );
    }
    ...
    protected boolean canGenerate(
        ...
        if (type == StructurePlacementType.FLOOR) return canGenerateFloor(
            level,
            pos,
            rotation
        );
        else if (type == StructurePlacementType.LAVA) return canGenerateLava(
            level,
            pos,
            rotation
        );
        else if (type == StructurePlacementType.UNDER) return canGenerateUnder(
            level,
            pos,
            rotation
        );
        else if (type == StructurePlacementType.CEIL) return canGenerateCeil(
            level,
            pos,
            rotation
        );
    ...
    protected boolean canGenerateFloorFreeAbove(
        LevelAccessor world,
        BlockPos pos,
        Rotation rotation
    ) {
    ...
    protected boolean canGenerateFloor(
        ...
        return (
            getAirFraction(world, pos, rotation) > 0.6 &&
            getAirFractionFoundation(world, pos, rotation) < 0.5
        );
    }

    protected boolean canGenerateLava(
        LevelAccessor world,
        BlockPos pos,
        Rotation rotation
    ) {
        ...
        return (
            getLavaFractionFoundation(world, pos, rotation) > 0.9 &&
            getAirFraction(world, pos, rotation) > 0.9
        );
    }

    protected boolean canGenerateUnder(
        LevelAccessor world,
        BlockPos pos,
        Rotation rotation
    ) {
    ...
    protected boolean canGenerateCeil(
        LevelAccessor world,
        BlockPos pos,
        Rotation rotation
    ) {
        ...
        return (
            getAirFractionBottom(world, pos, rotation) > 0.8 &&
            getAirFraction(world, pos, rotation) < 0.6
        );
    }
    ...
    protected float getAirFraction(
        LevelAccessor world,
        BlockPos pos,
        Rotation rotation
    ) {
        ...
        MutableBlockPos size = new MutableBlockPos().set(
            new BlockPos(structure.getSize()).rotate(rotation)
        );
        size.setX(Math.abs(size.getX()) >> 1);
        size.setZ(Math.abs(size.getZ()) >> 1);

        BlockPos start = pos.offset(-size.getX(), 0, -size.getZ());
        BlockPos end = pos.offset(
            size.getX(),
            size.getY() + offsetY,
            size.getZ()
        );
        ...
        for (int x = start.getX(); x <= end.getX(); x++) {
            ...
            for (int y = start.getY(); y <= end.getY(); y++) {
                ...
                for (int z = start.getZ(); z <= end.getZ(); z++) {
                    POS.setZ(z);
                    if (world.isEmptyBlock(POS)) airCount++;
                    count++;
                }
            }
        }
    ...
    private float getLavaFractionFoundation(
        LevelAccessor world,
        BlockPos pos,
        Rotation rotation
    ) {
        ...
        for (int x = start.getX(); x <= end.getX(); x++) {
            ...
            for (int z = start.getZ(); z <= end.getZ(); z++) {
                ...
                if (BlocksHelper.isLava(world.getBlockState(POS))) lavaCount++;
                count++;
            }
        }
    ...
    private float getAirFractionFoundation(
        LevelAccessor world,
        BlockPos pos,
        Rotation rotation
    ) {
        ...
        for (int x = start.getX(); x <= end.getX(); x++) {
            ...
            for (int y = start.getY(); y <= end.getY(); y++) {
                ...
                for (int z = start.getZ(); z <= end.getZ(); z++) {
                    POS.setZ(z);
                    if (world.getBlockState(POS).canBeReplaced()) airCount++;
                    count++;
                }
            }
        }
    ...
    private float getAirFractionBottom(
        LevelAccessor world,
        BlockPos pos,
        Rotation rotation
    ) {
