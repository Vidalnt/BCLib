...
public class PrintInfo {
    ...
    static int printDimensions(CommandContext<CommandSourceStack> ctx) {
        ...
        for (var serverLevel : ctx
            ...
            String output =
                ...
                " " +
                generator.getBiomeSource().toString().replace("\n", "\n     ");
            var cl = ChatFormatting.LIGHT_PURPLE;
            if (
                serverLevel
                    .dimension()
                    .identifier()
                    .equals(Level.OVERWORLD.identifier())
            ) cl = ChatFormatting.WHITE;
            else if (
                serverLevel
                    .dimension()
                    .identifier()
                    .equals(Level.NETHER.identifier())
            ) cl = ChatFormatting.RED;
            if (
                serverLevel
                    .dimension()
                    .identifier()
                    .equals(Level.END.identifier())
            ) cl = ChatFormatting.YELLOW;
            Component dimComponent = Component.literal(output).setStyle(
                Style.EMPTY.withBold(false).withColor(cl)
            );
    ...
    static int printMapValues(CommandContext<CommandSourceStack> ctx) {
        ...
        final Climate.TargetPoint sample = sampler.sample(
        ...
        result.append(
            Component.literal(output.toString()).setStyle(
