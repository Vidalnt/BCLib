...
public abstract class BaseWallBlock
    extends WallBlock
    implements
        DropSelfLootProvider<BaseWallBlock>,
        BlockModelProvider,
        BlockTagProvider
{
    ...
    public static BaseWallBlock from(Block source) {
        if (source instanceof BehaviourWood) return new BaseWallBlock.Wood(
            source
        );
        if (source instanceof BehaviourMetal) return new BaseWallBlock.Metal(
            source
        );
