...
public abstract class SimpleMaterialSlot<
...
> extends MaterialSlot<M> {
    ...
    protected BlockEntry getBlockEntry(M parentMaterial) {
        final BlockEntry entry = new BlockEntry(suffix, (c, p) ->
    ...
    protected @Nullable RecipeEntry getRecipeEntry(M parentMaterial) {
        return new RecipeEntry(suffix, (context, parentMaterial1, id) ->
            makeRecipe(context, parentMaterial1, id)
        );
    }
    ...
    public static <M extends ComplexMaterial> SimpleMaterialSlot<
        M
    > createBlockItem(
    ...
    public static <M extends ComplexMaterial> SimpleMaterialSlot<
        M
    > createBlockItem(@NotNull String suffix, Supplier<Block> maker) {
        return new SimpleMaterialSlot(suffix) {
            @Override
            protected @NotNull Block createBlock(
                ComplexMaterial parentMaterial,
